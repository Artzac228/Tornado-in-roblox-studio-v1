...
local function fx(size, color, transp, shape)
	...
	p.Size = size; p.Color = color; p.Transparency = transp
	p.Shape = shape or Enum.PartType.Block
	p.Anchored = true; p.CanCollide = false; p.CastShadow = false
	p.Material = Enum.Material.SmoothPlastic
	p.Parent = folder; fxSet[p] = true
	return p
end

local function smoke(part, op, color, rise)
	local s = Instance.new("Smoke")
	s.Enabled = false; s.Opacity = op or 0.4
	s.Size = 3; s.RiseVelocity = rise or 4
	s.Color = color or Color3.fromRGB(90,85,80)
	s.Parent = part; return s
end

local function particles(part, rate, speed, size, color, lifetime)
	local pe = Instance.new("ParticleEmitter")
	pe.Texture = "rbxasset://textures/particles/smoke_main.dds"
	pe.Rate = 0; pe.Speed = NumberRange.new(speed*0.2, speed)
...
local function addSmokePoint(ox, oy, oz, angle, radius, sz, op, clr, rate, rise)
	...
	p.Anchored=true; p.CanCollide=false; p.CastShadow=false
	p.Material=Enum.Material.SmoothPlastic
	p.CFrame=CFrame.new(ox,oy,oz); p.Parent=folder; fxSet[p]=true
...
snd.SoundId=CFG.SoundId; snd.Looped=true; snd.Volume=0
	...
	task.delay(D*0.3, function()
		for i, e in ipairs(smokeEmitters) do
			task.delay((i-1)*0.04, function()
				...
				task.delay(0.3, function() if e.pe then e.pe.Rate = math.floor(e.pe.Rate) end end)
				task.delay(0.05, function() if e.pe then e.pe.Rate = e.pe.Rate or 8 end end)
				-- Включаем Rate сразу
				if e.pe then e.pe.Rate = 0 end
				task.delay(0.2, function() if e.pe then e.pe.Rate = math.floor(8 * 0.5) end end)
				task.delay(0.7, function()
					if e.pe then
						-- Рассчитываем целевой rate по интенсивности
						local baseRate = 8
						e.pe.Rate = baseRate
					end
				end)
...
local dashTimer = 0; local isDashing = false; local dashLeft = 0
local randTimer = 0; local randIntv  = CFG.RandDirMin
...
local function nearestPlayer()
	...
	for _, pl in ipairs(Players:GetPlayers()) do
		...
		if h and hm and hm.Health > 0 then
			local dx = h.Position.X-PX; local dz = h.Position.Z-PZ
			local d  = math.sqrt(dx*dx+dz*dz)
			if d < bd then bd=d; bx=h.Position.X; bz=h.Position.Z end
		end
	end
	return bx, bz, bd
end
...
local function updateMovement(dt)
	...
	if mode=="nearest" then
		...
		if tx then
			...
			if len>0.1 then
				...
				if ml>0.01 then moveDX=moveDX/ml; moveDZ=moveDZ/ml end
			end
		else
			moveDX=moveDX*0.95; moveDZ=moveDZ*0.95
		end
		PX=PX+moveDX*spd*dt; PZ=PZ+moveDZ*spd*dt
	...
	elseif mode=="dash" then
		...
		if isDashing then
			...
			if dashLeft<=0 then isDashing=false; moveDX=0; moveDZ=0; dashTimer=0 end
		elseif dashTimer>=CFG.DashCooldown then
			local tx,tz,dist=nearestPlayer()
			if tx and dist>CFG.DashMinDist then
				local dx=tx-PX; local dz=tz-PZ
				local len=math.sqrt(dx*dx+dz*dz)
				moveDX=dx/len; moveDZ=dz/len
				isDashing=true; dashLeft=CFG.DashDuration; dashTimer=0
			else
	...
	elseif mode=="random" then
		...
		if randTimer>=randIntv then
			...
			local fx2=PX-START_X; local fz2=PZ-START_Z
			...
			moveDX=math.cos(a); moveDZ=math.sin(a)
		end
		PX=PX+moveDX*spd*dt; PZ=PZ+moveDZ*spd*dt
	end
end
...
local function updateAttraction(dt)
	...
	for _, child in ipairs(workspace:GetChildren()) do
		...
		local parts = {}
		if child:IsA("BasePart") then parts={child}
		...
		for _, p in ipairs(parts) do
			...
			local dx=p.Position.X-PX; local dz=p.Position.Z-PZ
...
local function capturePlayer(pl)
	...
	capturedSet[pl]=true
	local dx=hrp.Position.X-PX; local dz=hrp.Position.Z-PZ
...
local function capturePart(part)
	...
	table.insert(captured,{
		...
		wasA=wasA, wasC=wasC,
		spin=0,
		spinSpd=CFG.SpinSpeed*CFG.WindSpeed*(0.6+math.random()*0.8)*(math.random()>.5 and 1 or -1),
		tiltX=(math.random()-0.5)*0.6, tiltZ=(math.random()-0.5)*0.6,
	})
end
...
local function captureModel(model)
	...
	if not anchor then
		for _,p in ipairs(model:GetDescendants()) do
			if p:IsA("BasePart") then anchor=p;break end
		end
	end
	...
	for _,p in ipairs(model:GetDescendants()) do
		if p:IsA("BasePart") then
			local bv=attraced[p]; if bv and bv.Parent then bv:Destroy() end; attraced[p]=nil
			table.insert(infos,{part=p, wasA=p.Anchored, wasC=p.CanCollide,
	...
	local dx=anchor.Position.X-PX; local dz=anchor.Position.Z-PZ
	table.insert(captured,{
		type="model", obj=model, anchor=anchor, infos=infos,
		angle=math.atan2(dz,dx),
		height=math.max(anchor.Position.Y-PY,2),
		radius=math.min(math.sqrt(dx*dx+dz*dz),CFG.BaseRadius*1.1),
		vy=CFG.LiftSpeed*CFG.WindSpeed + math.random()*2,
		spin=0,
		spinSpd=CFG.SpinSpeed*CFG.WindSpeed*(0.5+math.random()*0.5)*(math.random()>.5 and 1 or -1),
		tiltX=(math.random()-0.5)*0.4, tiltZ=(math.random()-0.5)*0.4,
	})
end

-- ── Выброс ──────────────────────────────────────────────────
-- ПРОСТО убираем объект из таблицы и даём скорость.
-- Никаких Anchored изменений — объект уже НЕ анкорен.
...
local function releasePart(entry)
	...
	-- Даём скорость — объект полетит естественно под гравитацией
	...
	if entry.wasA then
		task.delay(5, function()
			if p and p.Parent and not capturedSet[p] then
				p.Anchored=true; p.CanCollide=entry.wasC
			end
		end)
	end
end
...
local function releaseModel(entry)
	...
	for _,info in ipairs(entry.infos) do
		local p=info.part; if not(p and p.Parent) then continue end
		objEjectCD[p]=tick()+3
		...
		if info.wasA then
			task.delay(5, function()
				if p and p.Parent and not capturedSet[p] then
					p.Anchored=true; p.CanCollide=info.wasC
				end
			end)
		end
	end
...
local function release(entry)
	if     entry.type=="player" then releasePlayer(entry)
	elseif entry.type=="part"   then releasePart(entry)
	elseif entry.type=="model"  then releaseModel(entry)
	end
end
	...
	if entry.type=="player" then
		...
		if CFG.PlayerDmgPerSec*CFG.WindSpeed>0 then
			...
			if entry.dmgAcc>=1 then
				local d=math.floor(entry.dmgAcc); entry.dmgAcc=entry.dmgAcc-d
				hum:TakeDamage(d)
				if hum.Health<=0 then release(entry); return true end
			end
		end

	elseif entry.type=="part" then
		local p=entry.obj; if not(p and p.Parent) then return true end
...
local function scan()
	...
	for _,pl in ipairs(Players:GetPlayers()) do
		...
		local dx=hrp.Position.X-PX; local dz=hrp.Position.Z-PZ
		if dx*dx+dz*dz<=pr2 then capturePlayer(pl) end
	end
	...
	for _,obj in ipairs(workspace:GetChildren()) do
		...
		if obj:IsA("BasePart") then
			...
			local dx=obj.Position.X-PX; local dz=obj.Position.Z-PZ
			if dx*dx+dz*dz<=pr2 then capturePart(obj) end

		elseif obj:IsA("Model") and CFG.GrabModels then
			-- Проверяем позицию модели
			local sumX,sumZ,n=0,0,0
			for _,p in ipairs(obj:GetDescendants()) do
				if p:IsA("BasePart") then sumX=sumX+p.Position.X; sumZ=sumZ+p.Position.Z; n=n+1 end
			end
			if n==0 then continue end
			local avgX=sumX/n; local avgZ=sumZ/n
			local dx=avgX-PX; local dz=avgZ-PZ
			if dx*dx+dz*dz<=pr2 then captureModel(obj) end
		end
	end
end
...
local conn=RunService.Heartbeat:Connect(function(dt)
	...
	if CFG.Duration>0 and elapsed>CFG.Duration then
		...
		snd:Stop(); folder:Destroy(); return
	end
	...
	-- Сканирование
	if scanTimer>=0.25 then scanTimer=0; scan() end
